...
async function scrapeUrl(url: string): Promise<string> {
  ...
  const contentSelector = 'article, main, [role="main"], .content, .post-content, .entry-content, body';
  const bodyText = $(contentSelector)
    .first()
    .text()
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 8000); // Cap per URL to avoid token overflow
